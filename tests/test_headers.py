...
class TestPointHeader:
    ...
    def test_change_vertical_reference(self, point_header):
        ...
        point_header.geoheader.change_vertical_reference(5709, 5710)
    ...
    def test_select_within_bbox(self, point_header):
        selected = point_header.geoheader.select_within_bbox(1, 1, 3, 3)
        assert len(selected) == 9

        # Test inverted selection: everything outside the bbox
        selected = point_header.geoheader.select_within_bbox(1, 1, 3, 3, invert=True)
    ...
    def test_select_with_points(self, point_header):
        ...
        buffer = 1.1
        selected = point_header.geoheader.select_with_points(selection_points, buffer)
        assert len(selected) == 16

        inverted_selection = point_header.geoheader.select_with_points(
            selection_points, buffer, invert=True
        )
        assert len(inverted_selection) == 9

        # Select with Shapely object
        selected = point_header.geoheader.select_with_points(
    ...
    def test_select_with_lines(self, point_header):
        ...
        buffer = 1
        selected = point_header.geoheader.select_with_lines(selection_gdf, buffer)
        assert len(selected) == 21

        inverted_selection = point_header.geoheader.select_with_lines(
            selection_gdf, buffer, invert=True
        )
        assert len(inverted_selection) == 4

        # Select with Shapely object
        selected = point_header.geoheader.select_with_lines(selection_lines[0], buffer)
        assert len(selected) == 13

        # Select with iterable
        selected = point_header.geoheader.select_with_lines(selection_lines, buffer)
        assert len(selected) == 21
    ...
    def test_select_within_polygons(self, point_header):
        ...
        selected = point_header.geoheader.select_within_polygons(selection_gdf)
        assert len(selected) == 3

        selected_inverted = point_header.geoheader.select_within_polygons(
            selection_gdf, invert=True
        )
        assert len(selected_inverted) == 22

        selected_buffered = point_header.geoheader.select_within_polygons(
            selection_gdf, buffer=0.1
        )
        assert len(selected_buffered) == 11

        selected_inverted_buffered = point_header.geoheader.select_within_polygons(
            selection_gdf, buffer=0.1, invert=True
        )
        assert len(selected_inverted_buffered) == 14

        # Shapely Polygon based selection
        selected = point_header.geoheader.select_within_polygons(selection_polygon)
        assert len(selected) == 3
    ...
    def test_select_by_depth(self, point_header):
        assert len(point_header.geoheader.select_by_depth(top_min=10)) == 16
        assert len(point_header.geoheader.select_by_depth(top_max=10)) == 10
        assert len(point_header.geoheader.select_by_depth(end_min=-10)) == 10
        assert len(point_header.geoheader.select_by_depth(end_max=-10)) == 16
        assert len(point_header.geoheader.select_by_depth(top_min=10, top_max=15)) == 6
        assert (
            len(
                point_header.geoheader.select_by_depth(
                    top_min=10, top_max=15, end_max=-12
                )
            )
            == 4
        )
    ...
    def test_select_by_length(self, point_header):
        assert len(point_header.geoheader.select_by_length(min_length=10)) == 21
        assert len(point_header.geoheader.select_by_length(max_length=30)) == 15
        assert (
            len(point_header.geoheader.select_by_length(min_length=10, max_length=30))
            == 11
        )
    ...
    def test_get_area_labels(self, point_header):
        ...
        # Return variant
        output = point_header.geoheader.get_area_labels(label_gdf, "id")
        pd.testing.assert_frame_equal(output.dropna(subset="id"), expected_output)
        # In-place variant
        point_header.geoheader.get_area_labels(label_gdf, "id", include_in_header=True)
    ...
    def test_get_area_labels_multiple(self, point_header):
        ...
        # Return variant
        output = point_header.geoheader.get_area_labels(label_gdf, ["id", "col2"])
        assert_almost_equal(output["id"].sum(), 11)
        assert_equal(output["col2"].value_counts()["string_data"], 11)
        # In-place variant
        point_header.geoheader.get_area_labels(
