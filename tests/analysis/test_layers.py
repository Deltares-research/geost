...
def layered_data():
    ...
    ics = [3, 2.6, 2, 2.6, 2, 2, 3, 2.6, 3, 2, 2.6, 3, 2.6, 2, 2, 2.6, 2, 2]
    return pd.DataFrame(
        {
            "nr": np.repeat(["a", "b"], [6, 12]),
            "surface": np.repeat([0.3, 0.4], [6, 12]),
            "top": top,
            "bottom": bottom,
            "lith": lith,
...
def discrete_data(layered_data):
    ...
    nlayers = [10, 4, 8, 2, 12, 20, 6, 2, 2, 4, 2, 12, 4, 4, 4, 1, 1, 10]
...
@pytest.mark.parametrize(
    "data, value, min_thickness, min_fraction, expected",
    [
        ...
        ("discrete_data", ["L", "Z"], 0.5, None, [0.5, 1.4]),
        ("layered_data", slice(1.2, 2.3), 0.5, 0.8, [0.7, 1.6]),  # slice for sand
        ("discrete_data", slice(1.2, 2.3), 0.5, 0.8, [0.7, 1.6]),  # slice for sand
    ],
    ids=[
        "layered_defaults",
        "discrete_defaults",
...
def test_get_layer_top(data, value, min_thickness, min_fraction, expected, request):
    ...
    assert_array_almost_equal(top["top"], expected)
...
def test_get_layer_top_errors(layered_data):
    ...
    with pytest.raises(
        ValueError, match="min_thickness must be positive and zero or greater."
    ):
