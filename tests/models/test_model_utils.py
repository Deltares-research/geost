...
@pytest.mark.unittest
def test_sample_along_line(voxelmodel, line):
    sampled = mut.sample_along_line(voxelmodel, line, nsamples=3)
    assert sampled.sizes == {"dist": 3, "z": 5}
    assert_array_almost_equal(sampled["dist"], [0.0, 1.414, 2.828], decimal=3)

    sampled = mut.sample_along_line(voxelmodel, line, dist=0.5)
    assert sampled.sizes == {"dist": 6, "z": 5}
    assert_array_almost_equal(sampled["dist"], [0, 0.5, 1, 1.5, 2, 2.5])


@pytest.mark.unittest
def test_sample_with_coords(voxelmodel, borehole_collection):
    coords = borehole_collection.header[["x", "y"]].values
    sampled = mut.sample_with_coords(voxelmodel, coords)
    assert sampled.sizes == {"idx": 4, "z": 5}
