...
class LayeredData(AbstractData):
    ...
    def __init__(self, df: pd.DataFrame):
        self._df = df
        self._df["depth"] = self._df["bottom"]

    @property
    def is_first_row_in_borehole(self) -> pd.Series:
        """
        Boolean mask indicating whether the row is the first row in a borehole.

        Returns
        -------
        pd.Series
            Boolean mask with True for the first row in each borehole.

        """
        return self._df["nr"] != self._df["nr"].shift(1)

    @property
    def is_last_row_in_borehole(self) -> pd.Series:
        """
        Boolean mask indicating whether the row is the last row in a borehole.

        Returns
        -------
        pd.Series
            Boolean mask with True for the last row in each borehole.

        """
        return self._df["nr"] != self._df["nr"].shift(-1)

    @property
    def layers_top(self) -> pd.Series:
        """
        Series containing the top depth of each layer in meters below the surface.

        Returns
        -------
        pd.Series
            Series with top depths for each layer.

        """
        # TODO change 0 to the actual surface, provided that a borehole does not neccessarily
        # start at 0 m depth.
        temp_top = (
            self._df["depth"]
            .shift(1, fill_value=0)
            .where(~self.is_first_row_in_borehole, 0)
        )
        return temp_top
    ...
    def append_vertical_data(
        self, other: "LayeredData", overwrite_duplicates: bool = False
    ) -> "LayeredData":
        """
        Append another dataframe of vertical data to this dataframe.

        Parameters
        ----------
        other : LayeredData
            The other dataframe to append to this dataframe.
        overwrite_duplicates : bool, optional
            If True, in case of duplicate object IDs and depth, the data in the
            other dataframe will overwrite data in this dataframe. By default False.

        Returns
        -------
        LayeredData
            A new dataframe of vertical data that is the result of appending the other
            dataframe.
        """
        # TODO add this whe we define reference systems for LayeredData/VerticalData
        # if other.horizontal_reference != self.horizontal_reference:
        #     other.change_horizontal_reference(self.horizontal_reference)

        combined_data = pd.concat([self._df, other]).reset_index(drop=True)

        if overwrite_duplicates:
            combined_data.drop_duplicates(
                subset=["nr", "depth"], keep="first", inplace=True
            )
        else:
            combined_data.drop_duplicates(
                subset=["nr", "depth"], keep="last", inplace=True
            )
    ...
    def to_header(
        ...
        # header = self._df[header_columns][self.is_first_row_in_borehole]
    ...
    def slice_depth_interval(
        ...
        sliced = sliced[
            (sliced["depth"] > upper_boundary) & (self.layers_top < lower_boundary)
        ]
        ...
        if update_layer_boundaries:
            ...
            # Can no longer update top when only using depth, unless we update surface
            # and end as well.
            sliced["surface"] = sliced["surface"] - upper_boundary
            sliced.loc[sliced["depth"] >= lower_boundary, "depth"] = lower_boundary
